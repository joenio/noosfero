class ProfileListBlock < Block
  ...
  def profile_list
    ...
    if !prioritize_profiles_with_image
      result = profiles.visible.all(:limit => limit, :order => 'updated_at DESC')
    elsif profiles.visible.with_image.count >= limit
      result = profiles.visible.with_image.all(:limit => limit * 3, :order => 'updated_at DESC')
    else
      result = profiles.visible.with_image + profiles.visible.without_image.all(:limit => limit - profiles.visible.with_image.count, :order => 'updated_at DESC')
    end
    result.sort_by{ rand }.slice(0..limit-1)
  end
