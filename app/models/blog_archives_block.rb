class BlogArchivesBlock < Block
  ...
  def visible_posts(user)
    blog.posts.select {|post| post.display_to?(user)}
  end
  ...
  def content(user)
    ...
    results = ''
    #TODO
    visible_posts(user).native_translations.group_by {|i| i.published_at.year }.sort_by { |year,count| -year }.each do |year, results_by_year|
