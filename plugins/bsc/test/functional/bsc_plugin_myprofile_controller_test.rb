...
class BscPluginMyprofileControllerTest < Test::Unit::TestCase
  ...
  should 'do not list profiles template on search' do
    e1 = Enterprise.create!(:name => 'Sample Enterprise 1', :identifier => 'sample-enterprise-1')
    e2 = Enterprise.create!(:name => 'Sample Enterprise 2', :identifier => 'sample-enterprise-2')
    assert_not_nil bsc.environment.enterprise_template
    assert_not_nil bsc.environment.inactive_enterprise_template

    get :search_enterprise, :profile => bsc.identifier, :q => 'ent'

    assert_no_match /#{bsc.environment.enterprise_template.name}/, @response.body
    assert_no_match /#{bsc.environment.inactive_enterprise_template.name}/, @response.body
  end
