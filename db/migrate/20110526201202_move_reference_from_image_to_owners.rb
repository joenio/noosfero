class MoveReferenceFromImageToOwners < ActiveRecord::Migration
  def self.up
    ...
    #end
    update("update profiles set profiles.image_id = images.id from profiles inner join images on profiles.id = images.owner_id")
    remove_colum :images, :owner_id
    remove_colum :images, :owner_type
  end
